        ...
        {loading ? <div>Chargement…</div> : (
          <div className="overflow-x-auto glass mt-4">
            <table className="table">
              <thead><tr><th>Titre</th><th>Slug</th><th>Visibilité</th><th>Diff.</th><th>Q</th><th>Attempts</th><th>Statut</th></tr></thead>
              <tbody>
                {rows.map(r=>(
                  <tr key={r.id}>
                    <td>{r.title}</td>
...
function QuickCreate({ onDone }){
  ...
  async function submit(){
    ...
    setLoading(false);
    onDone(ok, ok?"Créé ✅": j?.error || "Erreur");
  }
