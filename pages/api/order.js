...
export default async function handler(req, res) {
  try {
    ...
    if (!symbol || !["BUY", "SELL"].includes(SIDE) || !Number.isFinite(qtyNum) || qtyNum <= 0) {
      return res.status(400).send("Paramètres invalides");
    }

    // Frais (bps) depuis Settings (fallback 0 si table absente)
    let tradingFeeBps = 0;
    try {
      const s = await prisma.settings.findUnique({ where: { id: 1 } });
      tradingFeeBps = Number(s?.tradingFeeBps ?? 0);
    } catch {}
    ...
    if (!Number.isFinite(rate) || rate <= 0) return res.status(400).send("Taux FX indisponible");

    const notionalEUR = price * qtyNum * rate;
    const feeEUR = notionalEUR * (tradingFeeBps / 10000);

    if (SIDE === "BUY") {
      const costEUR = notionalEUR + feeEUR;
      ...
      await prisma.user.update({ where: { id: user.id }, data: { cash: user.cash - costEUR } });
      ...
      const proceedsEUR = notionalEUR - feeEUR;
      ...
      await prisma.user.update({ where: { id: user.id }, data: { cash: user.cash + proceedsEUR } });
    ...
    logError?.("order", e);
    return res.status(500).json({ error: "Échec ordre", detail: e?.message || String(e) });
  }
}