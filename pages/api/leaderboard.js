...
+      const d = new Date();
+      // jour de la semaine (1..7) en UTC (lundi=1)
+      const day = (d.getUTCDay() + 6) % 7; // 0=lundi-like
...
+    // on doit aussi couvrir les symboles tradés dans la période (pour qty_start)
+    let symbols = [...new Set(allPositions.map(p => p.symbol))];

+    // Si période ≠ saison : on charge les ordres depuis t0 (quantités/cash à corriger)
+    let ordersSinceT0 = [];
...
+        ccyBySymbol[s] = ccy;
+        priceEurBySymbol[s] = (Number.isFinite(px) ? Number(px) : 0) * rate; // now EUR
       } catch (e) {
     ...
     // 4) Equity (EUR) par utilisateur (NOW)
     const equityByUser = {};
...
+      }
+    } else {
+      // Jour/Semaine/Mois : reconstruire equity_t0
+      // 1) deltaQty depuis t0 et impact cash depuis t0 (approx en utilisant FX actuel)
+      const deltaQtyByUserSym = new Map(); // key `${userId}|${symbol}` -> number
+      const netCashImpactByUser = {}; // en EUR
+      // charge frais (bps) actuels si Settings existe, sinon 0
+      let feeBps = 0;
+      try {
...
+
+        // impact cash (BUY: -, SELL: +)
+        netCashImpactByUser[o.userId] = (netCashImpactByUser[o.userId] || 0) + (o.side === "BUY" ? -totalEUR : +totalEUR);
+      }
+
+      // 2) prix historiques à t0 (close du jour t0 ou précédent) + FX historique basique
...
+
+        // market value at t0 using qty_start
+        let mvStart = 0;
+        for (const s of symbols) {
...
+      }
+
+      // 4) perf = (equity_now - equity_start)/equity_start
+      for (const u of users) {
